...
* [p. 140] Alg 7.5 Update `greedy` to reflect the fact that `findmax` returns an action index instead of an action (thanks to Jushua Darrow)
...
* [p. 413] Alg 20.4 Update `greedy` to reflect the fact that `findmax` returns an action index instead of an action (thanks to Jushua Darrow)
